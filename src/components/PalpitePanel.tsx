...
export function PalpitePanel({ groupCode, matches, predictionsMap, teamsMap }: Props) {
  return (
    <div className="space-y-2">
      <h2 className="text-lg font-semibold">Grupo {groupCode}</h2>
      {matches.map(m => (
        <MatchCard key={m.id} match={m} prediction={predictionsMap.get(m.id)} teamsMap={teamsMap} />
      ))}
      {matches.length === 0 && (
