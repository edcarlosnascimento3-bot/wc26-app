...
const artilheiros = [
  ...
  {m:"K-6",t:"Polônia",j:"Lewandowski",g:1},{m:"K-6",t:"Irlanda",j:"Cullen",g:1},
...
async function main() {
  // First delete all existing goalscorers
  const { error: delErr } = await supabase.from("goalscorers").delete().gte("goals", 0);
  if (delErr) { console.error("Erro delete:", delErr.message); return; }
  ...
  for (const a of artilheiros) {
    ...
    const { error } = await supabase
      .from("goalscorers")
      .insert({ player_id: pid, team_id: tid, goals: a.g, match_id: a.m });
    if (error) { err++; console.error("Erro", a.t, a.j, a.m, error.message); }
    else ok++;
  }

  console.log(`Concluído! ${ok} registros inseridos, ${err} erros.`);
}
